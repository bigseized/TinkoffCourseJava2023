...
public class RequestCounterMetric implements Metric {
    private MetricsData metricsData;

    public void createMetric(MetricParams data) {
        Map<String, Integer> map = Arrays.stream(data.getParsedData().getLogs())
            .map(entry -> extractResource(entry.request().url()))
    ...
    private String extractResource(String url) {
